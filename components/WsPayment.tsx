...
export default function WsPayment({ email, darkMode }: { email: string, darkMode?: boolean }) {
  const [copied, setCopied] = useState(false)
  const [loading, setLoading] = useState(false)

  const textColor = darkMode ? 'white' : 'var(--ash)'
  const primaryBg = darkMode ? 'white' : 'var(--sumi)'
  const primaryText = darkMode ? 'black' : 'var(--washi)'
  const secondaryBorder = darkMode ? 'rgba(255,255,255,0.3)' : 'var(--fog)'
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      >
        {loading ? (
          <div style={{ width: 16, height: 16, border: `2px solid ${primaryText}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        ) : (
          <>
            ...
            <span style={{ flexShrink: 0 }}>Open Wealthsimple app</span>
          </>
        )}
      </button>
      ...
      >
        ...
        <span style={{ flexShrink: 0 }}>{copied ? 'Copied' : 'Copy Wealthsimple address'}</span>
      </button>
    </div>
  )
}
